...
const Profile = () => {
  ...
  const { data: UserPostData, isLoading } = useQuery<UserPostType[]>(['user-posts'], apis.getUserPosts);
  if (isLoading) {
    ...
    <>
      ...
      <div className='flex flex-col items-center pt-0.5'>
        ...
        <div className='flex justify-between w-[100%] mt-[80px] items-center'>
          <h1 className='font-bold text-start '>내 경기 ({data?.length})</h1>
          <Link to='/profile/userMatchMore'>
            <span className='text-sm'>더보기 +</span>
          </Link>
        </div>
        <UserMatches maxCount={5} />
        <h1 className='font-bold text-start w-[100%] mt-4'>내가 쓴 게시글 ({UserPostData?.length})</h1>
        <UserPost />
      </div>
    </>
  );
